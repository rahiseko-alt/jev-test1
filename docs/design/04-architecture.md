...
> **決定: Remix 3 は不採用。採用する構成は [05-stack.md](./05-stack.md) を参照。**
> 以下の Remix 3 に関する節は、不採用とした根拠の記録として残す。

## Remix 3 で作れるか: 条件付きで可能（ただし採用しない）
...
**結果として、この3条件は飲まない判断をした。**
Remix 2 の後継は Remix 3 ではなく React Router の framework mode であると公式が明言しており
<https://remix.run/blog/wake-up-remix>、そちらは v8.0.0 が 2026-06-17 に公開済みで実績もある。
詳細は [05-stack.md](./05-stack.md)。
...
└── web/           ← React Router。画面と入口の聞き取りだけ
```

`core` は単体で動かせて、テストもフレームワーク無しで書ける。
`web` は `core` を呼んで結果を並べるだけにする。画面の土台を差し替えても `core` は変えない。
